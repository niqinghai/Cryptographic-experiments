...
def main():
	...
	cypherTextsDecoded = map(methodcaller("decode", "hex"), cypherTexts) # 全部转换为ascii
    #两两异或
	crib = "There are two types of cyptography: one that allows the Government to use brute force to break the code, and one that requires the Government to use brute force to break you"#6的明文  已知明文攻击
	xorTextNum1 = 6      #6最长并且完全解密
	xorTextNum2 = 10     #需要解密的密文
	...
	for i in range(textLength):
    		...
    		# 做一个估计的猜测文本是否可读，如果是这样打印猜测（在适当的地方）
...
def strxor(a, b):     #不等长度字符串异或
    if len(a) > len(b):
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
    else:
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])

def isreadable(str): # 检查可读性（只允许字母数字字符和特殊字符）
	return bool(re.search('^[a-zA-Z0-9\., \'\"\-_\:\(\)]+$', str))

def printCodePos(i, code, maxLen, filler): # 输出猜测以代替消息
	prefix =  filler*i
	ln = len(code)
